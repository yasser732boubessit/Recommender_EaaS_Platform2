...
import com.hemza.master.recsClient.data.SessionExtractor;
 ...
 *
 */
public class CoOccurrence implements Algorithm {
	...
	public void handleEventNotification(Event eventData) {
		currenctUserSession = Collections.unmodifiableList(new ArrayList<>(sessionExtractor.addClick(eventData)));
	...
	public LongArrayList getRecommendations(Request currentRequest) {
		...
		for (int i = wholeSession ? 0 : (lastSession.size() - 1); i < lastSession.size(); i++) {
			...
			for (Entry<String, Integer> entry : m.entrySet()) {
				...
				/*
				 * if (!recommendableItems.contains(Long.parseLong(entry.getKey()))) { continue;
				 * }
				 */
				// -------------------------------------------
