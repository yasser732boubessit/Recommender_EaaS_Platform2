 ...
 *
 */
public class MostRecent implements Algorithm {
...
/* 
		if (results.size()>currentRequest.getLimit()) {
			return (LongArrayList) results.subList(0, currentRequest.getLimit());
		}else {
			return mostRecentItems;
		}
		*/
		if (results.size() > currentRequest.getLimit()) {
			return new LongArrayList(results.subList(0, currentRequest.getLimit()));
		} else {
			return mostRecentItems;
		}
