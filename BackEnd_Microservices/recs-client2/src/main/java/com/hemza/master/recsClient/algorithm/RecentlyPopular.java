...
public class RecentlyPopular extends MostPopular {
	...
	// a buffer of recent transactions so that click counts can be decreased when
	// they "leave" the time window
	...
	public void handleEventNotification(Event event) {
		...
		// filter all clicks from the transaction buffer that have left the time window
		// (e.g. when they occurred more than 1h ago)
		while ((recentTransactions.get(recentTransactions.size() - 1).getTime().getTime()
				- recentTransactions.get(0).getTime().getTime()) > filterTime) {
			// if the transactions get old, remove them from the list and decrease the
			// popularity count
